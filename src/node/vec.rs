...
pub(super) struct VecEdgeInfo<T> {
  pub(super) idx: usize,
  pub(super) node: Node<T>,
}

impl<T> EdgeInfo<T> for VecEdgeInfo<T> {
  fn node(&self) -> &Node<T> {
    &self.node
  }
}

pub(super) struct VecEdgeInfoRef<'a, T> {
  pub(super) idx: usize,
  pub(super) node: &'a Node<T>,
}

impl<'a, T> EdgeInfo<T> for VecEdgeInfoRef<'a, T> {
